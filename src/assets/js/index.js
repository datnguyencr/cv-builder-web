...
function resetUIFromCV(cv) {
    ...
    setupSectionHeaderEditing();
}

function setupSectionHeaderEditing() {
    renderWorkExpList();
    renderEducationList();
    renderSkillList();
    renderReferenceList();
...
workExpSaveBtn.addEventListener("click", () => {
    ...
    if (!title || !company) {
        showToast("Please fill role and company.");
        if (!title) {
            workExpTitleEl.focus();
        } else if (!company) {
...
eduSaveBtn.addEventListener("click", () => {
    ...
    if (!degree || !school) {
        showToast("Please fill degree and school.");
        if (!degree) {
            eduDegreeEl.focus();
        } else if (!school) {
            eduSchoolEl.focus();
...
document.addEventListener("DOMContentLoaded", async () => {
        ...
        document.querySelectorAll(".edit-section-title").forEach((btn) => {
            btn.onclick = (e) => {
                ...
                const newTitle = prompt(
                    "Rename Section (" + currentTitle + "):",
                    currentTitle,
                );
                if (newTitle && newTitle.trim() !== "") {
                    const trimmed = newTitle.trim();
                    titleEl.textContent = trimmed;
                    cvInfo.sectionHeaders[key] = trimmed;
                    previewPDF();
                }
            };
        });
    ...
    document.querySelectorAll(".section-toggle").forEach((toggle) => {
        toggle.addEventListener("click", (e) => {
            if (e.target.closest(".edit-section-title")) return;
